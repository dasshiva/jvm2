/* This code has been modified to add logs as well as some more utility methods 
 * for the jvm by @dasshiva */
...
fn read_constant_pool<T: Read>(data: &mut T) -> Result<ConstantPool, ClassFileError> {
    ...
    debug!("Size of constant pool: {constant_pool_count}");
    ...
    while i < constant_pool_count {
        ...
        let entry = match cp_type {
            7 => {
                debug!("Constant pool entry at index {i} : Class");
                ConstantPoolInfo::Class { name_index: read_u16(data)? } 
            },
            9 => { 
                debug!("Constant pool entry at index {i} : FieldRef");
            ...
            10 => { 
                debug!("Constant pool entry at index {i} : MethodRef");
                ConstantPoolInfo::MethodRef {
                class_index: read_u16(data)?,
                name_and_type_index: read_u16(data)?,
                }
            ...
            11 => {
                debug!("Constant pool entry at index {i} : InterfaceMethodRef");
                ConstantPoolInfo::InterfaceMethodRef {
                class_index: read_u16(data)?,
                name_and_type_index: read_u16(data)?,
                }
            ...
            8 => { 
                debug!("Constant pool entry at index {i} : String");
                ConstantPoolInfo::String {
                string_index: read_u16(data)?,
                }
            },
            3 => {
                debug!("Constant pool entry at index {i} : Integer");
                ConstantPoolInfo::Integer {
                data: unsafe { std::mem::transmute(read_u32(data)?) },
                }
            },
            4 => {
                debug!("Constant pool entry at index {i} : Float");
                ConstantPoolInfo::Float {
                data: unsafe { std::mem::transmute(read_u32(data)?) },
                }
            },
            5 => {
                debug!("Constant pool entry at index {i} : Long");
            ...
            6 => {
                debug!("Constant pool entry at index {i} : Double");
                let high = read_u32(data)?;
                let low = read_u32(data)?;
            ...
            12 => {
                debug!("Constant pool entry at index {i} : NameAndType");
            ...
            1 => {
                debug!("Constant pool entry at index {i} : Utf8");
            ...
            15 => {
                debug!("Constant pool entry at index {i} : MethodHandle");
            ...
            16 => {
                debug!("Constant pool entry at index {i} : MethodType");
                ConstantPoolInfo::MethodType {
                descriptor_index: read_u16(data)?,
                }
            },
            18 =>{
                debug!("Constant pool entry at index {i} : InvokeDynamic");
...
fn unpack<T> (res: Result<T, ClassFileError>) -> T {
    match res {
        Ok(s) => s,
        Err(e) => exit!("{}", e)
    }
...
impl MethodInfo {
    pub fn get_details(&self, cp: &ConstantPool) -> (String, String) {
      (unpack(cp.get_utf8_entry(self.name_index)), unpack(cp.get_utf8_entry(self.descriptor_index)))
    }
}
...
impl ClassFile {
    pub fn find_method(&self, name: &str, desc: &str) -> &MethodInfo {
        let nm = String::from(name);
        let ds = String::from(desc);
        for i in self.methods.iter() {
            let (n, d) = i.get_details(&self.constant_pool);
            if n == nm && d == ds {
                return i;
            }
        }

        exit!("Method {} not found", name)
    }
}
